...
import { updateUserStart,updateUserFailure,updateUserSuccess, deleteUserFailure, deleteUserStart, deleteUserSuccess, signOutUserStart, signInFailure, SignOutUserSuccess, signOutUserFailure } from '../redux/user/userSlice.js';
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <img 
        onClick={()=>fileRef.current.click()} className="rounded-full h-24 w-24 object-cover 
        cursor-pointer self-center mt-2" src={formData.avatar ||currentUser.avatar} alt='profile'
        />
        <p className='text-sm self-center'>
          {fileUploadError? 
            ...
            filePerc>0 && filePerc<100 ? (
              ...
              :
              filePerc===100 ? (
                <span className='text-green-500'>successfully 
                uploaded</span>)
                :("")
        }
        </p>
        ...
        p-3 uppercase hover:opacity-95 disabled:opacity-80 '>
          ...
          <Link className='bg-green-600 text-white p-3 rounded-lg uppercase text-center hover:opacity-95'
    ...
    <div className="flex flex-col gap-4">
      ...
      p-3 flex justify-between items-center gap-4'>
          ...
          <Link  className='flex-1 text-slate-700 
            font-semibold hover:underline truncate 'to={`/listing/${listing._id}`}>
            <p>{listing.name}</p>
          </Link>
